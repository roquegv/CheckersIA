...
public class Rules {
   ...
   /** 
    * Returns the list of all valid moves for player "color" on "board". If
    * a jump is possible, only jumps will be returned, since jumps are mandatory.
    */  
     ...
     for (int i = 1; i < 33; i++) {
         ...
         if ( (board.getChecker(c) != null) && 
            ...
            if (jumpExist)
               ...
               if (board.getChecker(c).findValidMoves(movelist, board))
                  jumpExist = true;
         }
      }
   ...
   /**
    * Uses the "Minimax with alpha-beta cutoff" algorithm to find
    * the best move for a player. However, unlike the normal minimax algorithm,
    * which searches an already built game tree, this algorithm builds and 
    * evaluates the game tree concurrently.
    */
   public static Board minimaxAB(Board board, int recursion, int player, 
      ...
      if (recursion > 0) {
         ...
         if (player == Checker.BLACK) {   // Black - min node.
            while (iterator.hasNext()) {
         ...
         if (player == Checker.BLACK) {   // Black - min node.
            while (iterator.hasNext())
               boardlist.add(minimax(executeMove(iterator.next(), board), 
                                     recursion - 1, opponent(player)));
            return boardlist.findBestBoard(Checker.BLACK);
         }
         else {      // White - max node.
            while (iterator.hasNext())
               boardlist.add(minimax(executeMove(iterator.next(), board), 
                                           recursion - 1, opponent(player)));
            return boardlist.findBestBoard(Checker.WHITE);
         }
      }
      else
         return board;   // Recursion done -> leaf in game tree.
   ...
   public static Board plusInfinityBoard() {
      ...
      for (int i = 1;i < 33; i++) {
         ...
         Checker WhiteKing = new WhiteChecker(c);
         WhiteKing.makeKing();
   ...
   public static Board minusInfinityBoard() {
      ...
      for (int i = 1;i < 33; i++) {
         Coordinate c = new Coordinate(i);
         Checker BlackKing = new BlackChecker(c);
         BlackKing.makeKing();
   ...
   public static boolean validWhiteMove(Coordinate from, Coordinate to, 
      ...
      return validUpMove(from, to, board) && 
             (board.getChecker(from).getColor() == Checker.WHITE);
   }
   ...
   public static boolean validBlackMove(Coordinate from, Coordinate to, 
      ...
      return validDownMove(from, to, board) &&
             (board.getChecker(from).getColor() == Checker.BLACK);
   }
   ...
   public static boolean validWhiteJump(Coordinate from, Coordinate to, 
                                        Board board) {
      return validUpJump(from, to, Checker.WHITE, board);
   }
   
   
   public static boolean validBlackJump(Coordinate from, Coordinate to, 
                                        Board board) {
      return validDownJump(from, to, Checker.BLACK, board);
   }
