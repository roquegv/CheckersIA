 ...
 */
public class NormalStateWhite implements CheckerState {   
   ...
   public boolean findValidMoves(final Checker c, final Board board, 
      ...
      if (! findValidJumps(c, board, validMoves)) {
         ...
         if (Rules.validWhiteMove(c.getPosition(), c.getPosition().upLeftMove(), 
             board))
            validMoves.add(new MoveNormal(c, c.getPosition().upLeftMove()));
            
         if (Rules.validWhiteMove(c.getPosition(), c.getPosition().upRightMove(), 
             board))
   ...
   public boolean findValidJumps(Checker c, Board board, MoveList validJumps) {
      boolean found = false;
      if (Rules.validWhiteJump(c.getPosition(), c.getPosition().upLeftJump(), 
          board)) {
         validJumps.add(new MoveJump(c, c.getPosition().upLeftJump()));
         found = true;
      }
      
      if (Rules.validWhiteJump(c.getPosition(), c.getPosition().upRightJump(), 
          board)) {
